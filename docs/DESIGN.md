...
**透明态 = 原生 widget 的单色态**（对齐系统桌面 widget 失焦后的观感）：环境键 `widgetIdle` 驱动，全部内容落到 `IdlePalette` 的单色亮度阶梯——**卡片里不存在任何色相**。原有用颜色承载的语义改成亮度承载：重要性 ★≥★★≥★★★ 三级灰阶、实际值 beat/miss 用明暗区分；强调色（时间分割线、选中筛选 chips）换中性色；旗帜 emoji 无法改色，用 `.grayscale(1)` 去饱和；连行分隔线、"高重要性"行底色、hover 底色也换成中性色。卡片边缘叠加"外暗内亮"双环**凝光描边**（任何壁纸上可见）。切换用 `withAnimation` 0.35s 过渡。

**亮度阶梯随外观翻转（本轮修复）**：单色阶梯不是"白字一套"。`IdlePalette` 的每一档都由 `NSColor(name:dynamicProvider:)` 按当前外观解析——深色 = 白 α 阶梯，浅色 = 黑 α 阶梯（同一张表两套锚点）。曾经的实现两态都用白字，于是浅色下必须靠一层黑补偿压暗卡片才能让白字看得见，结果是**浅色失焦态变成一块深灰板子**（用户反馈"跟 dark mode 一样黑"）。浅色用户看到的是浅色桌面与浅色原生 widget（实测系统提醒 widget 是近白的亮玻璃），白字卡片在这个语境里本身就是异类。现在浅色直接翻成深字，不再需要压暗。

**中性补偿层随外观翻转**：Liquid Glass `.clear` 会抬亮并去饱和背景（同一位置实测 +0.21 线性亮度，"奶感"来源），所以透明态在玻璃之上、内容之下叠一层 `Theme.idleVeil`：深色 = 黑 0.10（把卡片拉回壁纸自己的色调，白字有对比度），浅色 = 白 0.16（把卡片提成亮玻璃，深字有对比度，也更贴近浅色原生 widget 的近白观感）。实测（同一位置对比卡片内外）：浅色天空壁纸下卡片 +26 亮度、深灰背景（RGB 33）下卡片 123（深字对比度 ≈4.9）。凝光外暗环同样随外观降档（深色 0.20 / 浅色 0.10），否则浅色卡片上那圈黑边过重。

两点边界：① 补偿层只在非 `.solid` 档位生效，`.solid`（降低透明度）下卡片不透明、内容保持系统语义色；② 补偿是乘性的，无法完全复刻 widget 材质的"零抬亮"（亮背景上仍会略偏亮），要彻底一致只能等系统开放 widget 材质 API。
